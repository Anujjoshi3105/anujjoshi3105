...
const sendThankYouEmail = (name: string, email: string) => {
  const content = `
    ...
    <main style="padding: 30px; background-color: #fafafa;">
      ...
      <div style="margin-top: 40px; text-align: center;">
        <a href="${process.env.BASE_URL}" style="display: inline-block; padding: 12px 20px; background-color: #4F46E5; color: #fff; text-decoration: none; border-radius: 6px; font-size: 16px; font-weight: 700; text-transform: uppercase;">
          Contact Us
        </a>
      </div>
    </main>
  ...
  return createMailOptions(
    email,
    `${process.env.BASE_NAME} - Thank You for Reaching Out`,
    createEmailTemplate(content)
  );
};

export const sendEmail = async (name: string, email: string, msg: string) => {
  try {
    await Promise.all([
      transporter.sendMail(sendToRecipient(name, email, msg)),
